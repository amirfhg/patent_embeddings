...
The figure below is a box plot of share turnover across deciles based on $$\[
Cosine(\vec{\text{patent}}, \vec{\Delta_{12-60}})
\]$$ in the sample. The first plot presents the same-day share turnover as the patent grant news is announced. Other plots present share turnovers in the following two days. As we can see the median share turnover is positively associated with $$\[
Cosine(\vec{\text{patent}}, \vec{\Delta_{12-60}})
\]$$. This suggests that the share turnover -- a proxy for patent value based on Kogan, Papanikolaou, Seru, and Stoffman (2017) -- is positively correlated with patent's cosine similarity with $$\vec{\Delta_{12-60}}$$.
