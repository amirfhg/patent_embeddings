...
Alternatively, one can rely on LLM's ability to read patents and identify innovation trends as a complement to the methodology discussed above. This way the model's response benefits from focusing on the content of a carefully selected set of patents -- among hundreds of thousands of patents -- based on our methodology. The aim here is to improve LLM's ability to identify emerging innovation hotspots without forcing it to read unimportant (cos(```latex
\[
\vec{\text{innovation trend}}
\], )) patents. This reduces the noise and improves the relevance and interpretability in LLM's response.
